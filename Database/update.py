try:
    from Database.db import Account, db, Device, AccountStatus
except ImportError:
    from .db import Account, db, Device, AccountStatus
...
def update_account_status(account_username, account_atr_value):
    AccountStatus.query.filter_by(account_username=account_username).update(dict(status=account_atr_value))
    db.session.commit()
